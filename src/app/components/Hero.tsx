...
export const Hero = () => {
  return (
    <section id="home" className="relative flex min-h-[85vh] flex-col items-center justify-center overflow-hidden bg-[#FDF8E8] px-4 text-center">
      ...
      <div className="relative z-10 flex flex-col items-center">
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, type: 'spring' }} className="mb-8 max-w-2xl">
          <img src={logo} alt="ألعاب زمان" className="w-full drop-shadow-[4px_4px_0px_#E08C36]" />
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9, duration: 0.8 }} className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
          <motion.div whileHover={{ scale: 1.05, boxShadow: '6px 6px 0px #2D3436' }} whileTap={{ scale: 0.95, boxShadow: '2px 2px 0px #2D3436' }}>
            <Link to="/games" className="flex items-center gap-2 rounded-2xl border-4 border-[#6A8D56] bg-[#6A8D56] px-8 py-4 font-['Lalezar'] text-2xl text-[#FDF8E8] shadow-[8px_8px_0px_#2D3436] transition-all">
              <Gamepad2 className="h-8 w-8" />
              ابدأ اللعب الآن
            </Link>
          </motion.div>

          <motion.div whileHover={{ scale: 1.05, boxShadow: '6px 6px 0px #E08C36' }} whileTap={{ scale: 0.95, boxShadow: '2px 2px 0px #E08C36' }}>
            <Link
              to="/games"
